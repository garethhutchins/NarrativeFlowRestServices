...
def train_table(request):
   #Check that there is a file:
    ...
    if model_type == 'K-MEANS':
        #Check the Num Topics is > 0 
        if num_topics <1:
            response = {"Message" : "Number of Topics must be greater than 0 for the K-Means model"}
            status_code = status.HTTP_400_BAD_REQUEST
            return response, status_code
        model, plot_image, vectorizer = train_kmeans(df,num_topics)
        plot_image.savefig(model_name + '.png')
    ...
    if model_type == "TF-IDF":
        ...
        if 'label_column' in request._full_data:  
            ...
            if status_code != 200:
                ...
                plot_image.savefig(model_name + '.png')
                #Convert the list to a dictionary for a nicer display
                d_lbls = dict(zip(list(range(1,len(lbls)+1)),lbls))
                #Save the results
                num_topics = len(lbls)
                tfidf_labels = {'score': score,'labels':d_lbls}
